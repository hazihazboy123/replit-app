        ...
        for card_info in cards_data:
            ...
            if image_data:
                if isinstance(image_data, dict):
                    # Handle image object with URL and caption
                    ...
                    if image_url:
                        ...
                        if downloaded_filename:
                            if image_caption:
                                image_content = f'<img src="{downloaded_filename}" alt="{image_caption}"><br><em>{image_caption}</em>'
                            else:
...
def generate_synaptic_recall_name(cards):
    ...
    if not cards:
        return "synapticrecall_medicalcards"
    
    # Medical topic keywords for intelligent naming
    topic_keywords = {
        'cardiology': ['heart', 'cardiac', 'cardiovascular', 'coronary', 'myocardial', 'ecg', 'ekg'],
        'neurology': ['brain', 'neurological', 'spinal', 'cord', 'nerve', 'neural', 'spinothalamic'],
        'anatomy': ['anatomical', 'structure', 'muscle', 'bone', 'tissue', 'organ'],
        'pharmacology': ['drug', 'medication', 'dosage', 'pharmaceutical', 'therapeutic'],
        'endocrinology': ['hormone', 'endocrine', 'diabetes', 'thyroid', 'insulin'],
        'respiratory': ['lung', 'respiratory', 'breathing', 'pulmonary', 'oxygen'],
        'hematology': ['blood', 'hematological', 'anemia', 'bleeding', 'coagulation'],
        'immunology': ['immune', 'antibody', 'antigen', 'vaccination', 'allergy'],
    ...
    all_text = ""
    for card in cards:
        for field in ['front', 'back', 'question', 'answer', 'explanation']:
            text = card.get(field, '')
            if isinstance(text, str):
                all_text += f" {text.lower()}"
        
        # Include vignette content
        vignette = card.get('vignette', {})
        if isinstance(vignette, dict):
            for v_field in ['clinical_case', 'explanation']:
                v_text = vignette.get(v_field, '')
                if isinstance(v_text, str):
                    all_text += f" {v_text.lower()}"
    ...
    if topic_scores:
        ...
        return f"synapticrecall_{best_topic}"
...
def api_health():
    ...
    return jsonify({
        'status': 'healthy',
        'service': 'Enhanced Medical Flashcard Converter',
        'version': '4.0.0',
        'timestamp': time.time()
    })
