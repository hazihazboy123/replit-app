...
def parse_markdown_json(raw_input):
    ...
    if json_match:
        ...
        app.logger.debug("Extracted JSON from markdown wrapper")
    else:
...
def api_flexible_convert():
    """
    Simple endpoint for converting n8n markdown-wrapped JSON to Anki decks.
    Expects: ```json { "cards": [...] } ```
    """
    if request.method == 'OPTIONS':
        return '', 200
    ...
    try:
        ...
        app.logger.info(f"Received {len(raw_data)} bytes")
        ...
        try:
            ...
            return jsonify({
                ...
                'preview': raw_data[:300]
            }), 400
        
        if not cards:
            return jsonify({'error': 'No cards found in data'}), 400
        
        app.logger.info(f"Parsed {len(cards)} cards")
        
        # Generate smart deck name from tags
        deck_name = generate_smart_deck_name(cards)
        app.logger.info(f"Deck name: '{deck_name}'")
        ...
        result = {
            ...
            'download_url': download_url,
            'full_download_url': full_url,
            'storage_type': 'supabase' if supabase_result and supabase_result.get('success') else 'local',
            'message': f'Successfully generated deck "{deck_name}" with {len(cards)} cards'
        }
...
def index():
    ...
    <body>
        ...
        <p>Version 11.0.0 - Simplified with json_repair</p>

        <div class="features">
            <h3>Features:</h3>
            <ul>
                <li><strong>100% Reliable Parsing</strong> - json_repair handles all LLM edge cases</li>
                <li>Pure HTML preservation - no style stripping</li>
                <li>Full cloze card support</li>
                <li>Smart deck naming from content tags</li>
                <li>Supabase permanent storage</li>
                <li>Images displayed at 70% width</li>
                <li>Clinical vignettes and mnemonics preserved</li>
            </ul>
        </div>
